...
| 底色 | 含义 |
|---|---|
| 绿色（brand） | 正常出勤 / 今天 / 请假——请假不再单独给颜色，并入"正常"这一档，具体是不是请假、请的什么假，点开当天详情看 |
| 粉红（rose） | 异常——迟到 / 早退 / 缺卡三者共用同一个底色 |
| 灰色（slate） | 没排班——法定节假日 + 排班休息日统一算，两者本质都是"这天不用出勤"，不再用靛蓝单独区分节假日 |

日历格子**不再叠加任何角标小圆点**（迟到/早退/加班原本各有一个圆点，现已全部去掉）。原因：
迟到/早退的圆点定位用的是"当天最早上班卡/最晚下班卡"这个全天概念，多段班次下会把点标到错误的段上
（比如上午段准时、下午段迟到，圆点却出现在上午段记录旁边）；与其展示一个可能定位错误的细分标记，
不如统一只用底色表示"有没有异常"。加班圆点虽然没有这个定位问题，但为了让日历格子的视觉规则保持
一致（要么都用底色，要么都不用点），一并去掉了。具体是迟到/早退/加班/请假类型，点开当天详情看文字标签。
